...
struct pasticceria_t{
    sem_t mutex;
    sem_t clienti, cuoco, commesso, semAttesaClienti;
...
void init_pasticceria(struct pasticceria_t *p){
    ...
    sem_init(&p->semAttesaClienti, 0, 0);
...
void commesso_prendo_torta(struct pasticceria_t *p){
    ...
    printf("Sono il commesso e sto prendendo la torta\n");
    sem_wait(&p->commesso);
